...
async function startServer() {
  ...
  app.post('/api/synthesize', async (req, res) => {
    try {
      ...
      Return ONLY a valid JSON object with this shape: { "blueprint": "text", "protocol": "text" }. Do not use markdown blocks.`;
  ...
  app.get('/api/health', (req, res) => {
    res.json({ environment: process.env.NODE_ENV || 'development', status: 'ok' });
  });
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    console.log('Starting in PRODUCTION mode (serving static files from /public)...');
